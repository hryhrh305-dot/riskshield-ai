  ...
  try {
    ...
    switch (event.type) {
      case "checkout.completed": {
        ...
        await getSupabaseAdmin().from("payments").update({ status: "completed" }).eq("provider_checkout_id", event.checkout_id);
        break;
      }
      case "subscription.cancelled": {
        const subId = event.subscription_id;
        if (!subId) break;
        const { data: sub } = await getSupabaseAdmin().from("subscriptions").select("user_id").eq("provider_subscription_id", subId).single();
        if (sub) {
          await getSupabaseAdmin().from("profiles").update({ subscription_status: "cancelled", updated_at: new Date().toISOString() }).eq("id", sub.user_id);
        }
        break;
      }
      case "subscription.expired": {
        ...
        const { data: sub } = await getSupabaseAdmin().from("subscriptions").select("user_id").eq("provider_subscription_id", subId).single();
        if (sub) {
          await getSupabaseAdmin().from("profiles").update({ plan: "free", subscription_status: "expired", subscription_end: null, updated_at: new Date().toISOString() }).eq("id", sub.user_id);
        }
        break;
      }
    }
