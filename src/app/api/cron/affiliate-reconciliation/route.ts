...
export async function POST(request:Request){
  ...
  const [sales,decisions,ledger,batches,incidents]=await Promise.all([
    admin.from("affiliate_sales").select("id",{count:"exact",head:true}).eq("program_id","secwyn-india").eq("status","qualified"),
    admin.from("affiliate_commission_decisions").select("id,amount_minor,affiliate_sales!inner(status)").eq("program_id","secwyn-india").eq("affiliate_sales.status","qualified"),
  ...
  const queryError=sales.error||decisions.error||ledger.error||batches.error||incidents.error;if(queryError) return NextResponse.json({error:"Reconciliation unavailable."},{status:503});
  const decisionAmount=sum(decisions.data||[]);const ledgerAmount=sum(ledger.data||[]);const payoutAmount=sum(batches.data||[]);const matched=(sales.count||0)===(decisions.data||[]).length&&decisionAmount===ledgerAmount&&(incidents.count||0)===0;
  const evidence={sales:sales.count||0,decisions:(decisions.data||[]).length,decisionAmountMinor:decisionAmount.toString(),ledgerAmountMinor:ledgerAmount.toString(),payoutAmountMinor:payoutAmount.toString(),openHighSeverityIncidents:incidents.count||0};
